...
class PyWV:
    def __init__(self, q, start_ev, exit_ev, loaded, emit_queue, return_queue, html, debug,
        ...
        self.windows = []
        self.create_window(width, height, x, y, screen, on_top, maximize, title, frameless, easy_drag)

        start_ev.wait()
        webview.start(debug=debug)
        self.exit.set()

    def create_window(self, width, height, x, y, screen=None, on_top=False, maximize=False, title='', frameless=False, easy_drag=False):
        ...
        self.windows.append(webview.create_window(
            title, html=self.html, js_api=self.callback_api, frameless=frameless, easy_drag=easy_drag,
...
class Chart(abstract.AbstractChart):
    ...
    def __init__(self, width: int = 800, height: int = 600, x: int = None, y: int = None, title: str = '',
                 ...
                 scale_candles_only: bool = False, position: FLOAT = 'left', frameless: bool = False, easy_drag: bool = False):
        ...
        if self._i == 0:
            ...
            super().__init__(window, inner_width, inner_height, scale_candles_only, toolbox, position=position)
            self._q.put(('create_window', (width, height, x, y, screen, on_top, maximize, title, frameless, easy_drag)))
