...
class Table(Pane, dict):
    ...
    def update_cell(self, row_id, column, value):
        """Update a cell in the table and update the value in the chart table

        Args:
            row_id : the id of the row to update
            column : the column to update
            value : the value to update the cell to
        """
        self.run_script(f'{self.id}.updateCell("{row_id}", "{column}", "{value}")')
        self[row_id][column] = value

    def delete_row(self, row_id):
        """Delete a row from the table and the chart table

        Args:
            row_id : the id of the row to delete
        """
        self.run_script(f"{self.id}.deleteRow('{row_id}')")
        self.pop(row_id)
